...
class purchase_transactions_views(APIView):
    ...
    def post(self, request):
        ...
        combine_date = Purchase_det_new.objects.filter(
            Q(user_id=userid), Q(date__range=[from_date, to_date]))
        serializers = Purchase_transactions_serializer(
            data=combine_date, many=True)
...
class cuisine_transactions_views(APIView):
    ...
    def post(self, request):
        ...
        from_date = request.data.get('from_date')
        to_date = request.data.get('to_date')
        combine_date = cuisine_consumption_det.objects.filter(
            Q(user_id=userid), Q(date__range=[from_date, to_date]))
        serializers = Cuisine_transactions_serializer(
            data=combine_date, many=True)
        if serializers.is_valid():
            serializers.save()
        return Response({'status': 'SUCESS', 'data': serializers.data}, status=200)

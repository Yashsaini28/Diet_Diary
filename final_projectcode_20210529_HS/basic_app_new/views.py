...
from basic_app_new.models import Food_diary_new
...
for i in range(3,len(z)):
    if(i%3==0):
        ...
        ss=re.search('<FM_CALORIE_SATURATED_FATS>(.*)</FM_CALORIE_SATURATED_FATS>', z[i])
...
foodlist['image_path']=foodlist['image_path'].apply(lambda x: x.replace('//', '/'))
foodlist['image_path']=foodlist['image_path'].apply(lambda x: x.replace('http:/pancare.', 'http://tpancare.'))
